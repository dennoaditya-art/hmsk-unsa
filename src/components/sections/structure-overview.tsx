      ...
      <div className="relative mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 py-10 sm:py-16 lg:py-20">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} variants={staggerContainer} className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          <motion.div variants={fadeUp}>
            <span className="label text-[11px] text-muted-foreground">STRUKTUR PENGURUS</span>
        ...
        <div className="mt-10">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={lineDraw} className="hidden lg:block h-[2px] bg-border w-full max-w-[880px] mx-auto rounded-full origin-left" />
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }} variants={staggerContainer} className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 max-w-6xl mx-auto">
            {pengurus.map((leader) => (
              <motion.div key={leader.name} variants={scaleIn} className="relative">
                <motion.div initial={{ scaleY: 0 }} whileInView={{ scaleY: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.4 }} style={{ originY: 0 }} className="hidden lg:block absolute -top-6 left-1/2 h-6 w-[2px] bg-border -translate-x-1/2 rounded-full" />
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }} className="mt-8 sm:mt-10 flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2.5 sm:gap-3">
          <Link href="/struktur" className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-foreground px-6 py-3.5 text-sm font-bold text-background transition hover:opacity-90 min-h-12 sm:min-h-0">
            Lihat detail semua pengurus <ArrowRight className="h-4 w-4" aria-hidden />
          </Link>
          <Link href="/tentang" className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-card px-6 py-3.5 text-sm font-bold text-foreground transition hover:bg-muted min-h-12 sm:min-h-0">
            <Users className="h-4 w-4" aria-hidden /> Tentang HMSK
          </Link>
        </motion.div>

        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="label text-center text-[11px] text-muted-foreground mt-6">5 PENGURUS INTI • 3 PROKER AKTIF • 1 VISI</motion.p>
      </div>
    </section>
  );
}
