  ...
  return (
    <section id="tentang" className="relative bg-background border-y border-border overflow-hidden">
      ...
      <div className="relative mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8 py-10 sm:py-16 lg:py-20">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={staggerContainer} className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-12">
          <motion.div initial={{ opacity: 0, y: 20, rotate: -0.4 }} whileInView={{ opacity: 1, y: 0, rotate: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} whileHover={shouldReduce ? undefined : { y: -4 }} className="lg:col-span-7 rounded-[28px] bg-[#020617] text-white overflow-hidden relative p-6 sm:p-8">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="lg:col-span-5 grid gap-4">
            {[
              ...
              <motion.div key={v.title} variants={scaleIn} whileHover={shouldReduce ? undefined : { y: -4, scale: 1.015 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="rounded-[24px] border border-border bg-card p-5 flex gap-4 hover:border-primary/40 hover:shadow-md transition-shadow">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }} className="lg:col-span-12 rounded-[28px] border border-border bg-card p-6 sm:p-8">
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-[320px] shrink-0">
                ...
                <a href="/tentang" className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold tracking-widest text-primary hover:text-foreground group">
                  SELENGKAPNYA <motion.span className="inline-flex" whileHover={{ x: 3 }}><ArrowUpRight className="h-3 w-3" /></motion.span>
                </a>
              </div>
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid gap-3 sm:grid-cols-2 flex-1">
                {orgProfile.mission.map((m, i) => (
                  <motion.div key={i} variants={fadeUp} whileHover={shouldReduce ? undefined : { y: -2 }} className="group flex gap-3 rounded-2xl bg-muted/50 border border-border p-4 hover:bg-card hover:border-primary/30 hover:shadow-sm transition">
