    ...
    <main className="pt-[72px]">
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
        <div className="grid gap-6 lg:grid-cols-12">
          <div className="lg:col-span-7 rounded-[28px] bg-foreground p-6 sm:p-8 text-background relative overflow-hidden">
          ...
          <div className="lg:col-span-5 rounded-[28px] border border-border bg-card p-6 sm:p-8">
            <h2 className="display text-xl font-bold tracking-tight text-foreground">Misi</h2>
            <ul className="mt-4 space-y-3">
              {orgProfile.mission.map((m, i) => (
                <li key={i} className="flex gap-3 rounded-2xl bg-background border border-border p-3">
        ...
        <div className="mt-10">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground max-w-2xl">Detail proker sesuai arahan pengurus — dibina mentor yang ngerti teknis & bisnisnya.</p>

          <div className="mt-6 grid gap-5 lg:grid-cols-3">
            <Card className="border-border bg-card hover:border-primary/30 hover:shadow-md transition overflow-hidden">
              <CardContent className="p-6">
                ...
                <h3 className="mt-4 text-base font-bold text-foreground">{prokers[0].title}</h3>
                <p className="mono text-[11px] font-bold tracking-widest text-primary">DIBINA MAS DENNO</p>
                ...
                <div className="mt-4 inline-flex items-center gap-1 rounded-full bg-primary px-3 py-1 mono text-[11px] font-bold text-foreground">
                  <span className="h-1.5 w-1.5 rounded-full bg-foreground animate-pulse" /> BERJALAN
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-border bg-card hover:border-primary/30 hover:shadow-md transition overflow-hidden">
              <CardContent className="p-6">
                ...
                <h3 className="mt-4 text-base font-bold text-foreground">{prokers[1].title}</h3>
                <p className="mono text-[11px] font-bold tracking-widest text-primary">DIBINA DICTO</p>
                ...
                <div className="mt-4 inline-flex items-center gap-1 rounded-full bg-foreground border border-primary/30 px-3 py-1 mono text-[11px] font-bold text-primary">LAB AKTIF</div>
              </CardContent>
            </Card>
            ...
            <Card className="border-border bg-card hover:border-primary/30 hover:shadow-md transition overflow-hidden">
              <CardContent className="p-6">
                ...
                <h3 className="mt-4 text-base font-bold text-foreground">{prokers[2].title}</h3>
                <p className="mono text-[11px] font-bold tracking-widest text-muted-foreground">TIM HMSK • NEXT MEET</p>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">{prokers[2].description}</p>
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {prokers[2].highlights.map((h) => (
                    <span key={h} className="rounded-full bg-background border border-border px-2.5 py-1 mono text-[11px] text-muted-foreground">{h}</span>
                  ))}
                </div>
                <div className="mt-4 inline-flex items-center gap-1 rounded-full bg-muted border border-border px-3 py-1 mono text-[11px] font-bold text-muted-foreground">
